...
public class kpayne4_BattleshipPlayer implements BattleshipPlayer {
    ...
    public Board hideShips() {
        ...
        Board myBoard = null;
        try {
            if (firstBoardCreate) {                         //For the first board generated it is compared against a 
                previousBoard = new char[10][10];           //blank board and is set to false after first game
            ...
            /*boardsMatch is true and keeps regenerating a new board while the previous 
            board matches the new one            
            */
            boolean boardsMatch;
            do {
                ...
                I chose a pattern where no two ships will be next to each other and set a random x,y for the tope corner
                within (1,1) and (3,3) so the ships won;t be on the edge 
                */
                if (rot == 0) {
                ...
                boardsMatch = true;
                /*sets boardsMatch to false when the previous board doesnt match the new board
                and exits the do while when the boards don't match
                */
                for (int i = 0; i < HEIGHT; i++) {
                    for (int j = 0; j < WIDTH; j++) {
    ...
    private void getGuess(int shipType) {
        ...
        /* Once a ship is hit once it will guess a random spot to fire on
        that's  x+1, x-1, y+1, or y-1
        */
        if (numShipHits[shipType] == 1) {
            ...
            } while (myGuesses[guessX][guessY] != 'x');     //only guesses when the square is a valid space
        } else {
            /* After the second hit there is a check for if the ship is vertical or horizontal (by checking if x's are equal or if y's are equal) 
            the program will default to using the first hit to check + or - 1 vertivally or horizontally
            */
            if (useBegin == true) {
